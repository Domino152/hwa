...
import { Users, UserCheck, UserX, TrendingUp, ClipboardCheck, Clock } from "lucide-react";
import { formatDate } from "@/lib/academicYear";
        ...
        <CardContent>
          {summary?.recentRecords && summary.recentRecords.length > 0 ? (
            <div className="space-y-3">
              {summary.recentRecords.map((record, idx) => (
                <div
                  key={`${record.studentId}-${record.subject}-${idx}`}
                  className="flex items-center justify-between p-3 rounded-lg bg-muted/50"
                >
                  <div>
                    <p className="font-medium font-mono text-sm">{record.studentId}</p>
                    <p className="text-sm text-muted-foreground">
                      {record.subject} • {formatDate(record.date)}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    {record.status === "late" && <Clock className="h-4 w-4 text-amber-500" />}
